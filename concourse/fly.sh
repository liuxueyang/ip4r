...
case ${pipeline_config} in
  ...
  commit)
      if [ -z "${pipeline_name}" ]; then
          pipeline_name="COMMIT:ip4r:gpdb"
      fi
  ...
  release)
      ...
      if [ -z "${pipeline_name}" ]; then
          pipeline_name="RELEASE:ip4r:${branch}"
      fi
...
yml_path="/tmp/ip4r_pipeline.yml"
